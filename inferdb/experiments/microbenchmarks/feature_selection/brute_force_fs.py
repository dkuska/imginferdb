...
from src.optimizer import Optimizer, Problem
...
def get_pipeline_ccf(model):
    ...
    column_transformer = ColumnTransformer(
        transformers=[("num", numerical_transformer, [i for i in range(29)])],
        remainder="passthrough",
    )
    pipeline = Pipeline(
        steps=[("column_transformer", column_transformer), ("clf", model)]
    )
...
def get_kv_tuples_ccf(
    trained_pipeline, training_features, path_to_data, target_feature
):
    ...
    encoded_training_set = encoder.transform_dataset(
        X_train.to_numpy(), [i for i in range(X_train.shape[1])]
    )
    end = time.time() - start
    my_problem = Problem(
        encoded_training_set, y_train, encoder.num_bins, "classification", 1
    )
    encoding_time = end

    encoded_test_set = encoder.transform_dataset(
        X_test.to_numpy(), [i for i in range(X_test.to_numpy().shape[1])]
    )
    ...
    brute_force_df = brute_force_df.sort_values(
        ["IV"], ignore_index=True, ascending=False
    )
    ...
    greedy_agg_df = encoded_df.groupby(
        [i for i in range(len(solution))], as_index=False
    )[target_variable_number].mean()
    greedy_tuples = greedy_agg_df.to_numpy()

    brute_force_agg_df = encoded_df.groupby(
        [i for i in range(len(brute_force_solution))], as_index=False
    )[target_variable_number].mean()
    ...
    results = [
        (
            "Greedy",
            solution,
            my_optimizer.greedy_iv,
            sys.getsizeof(greedy_index),
    ...
    results.append(
        (
            "Brute Force",
            brute_force_df["Features"][0],
            brute_force_df["IV"][0],
            sys.getsizeof(bruteforce_index),
            brute_force_end,
        )
    )

    results_df = pd.DataFrame(
        results, columns=["Solution", "Features", "IV", "Size (B)", "Runtime"]
    )

    return results_df


def get_kv_tuples_nyc(
    trained_pipeline,
    training_features,
    path_to_data,
    target_feature,
    ...
    results_df = pd.DataFrame(
        results, columns=["Solution", "IV", "Size (B)", "Runtime"]
    )
...
def build_deterministic_trie(
    ...
    greedy_agg_df = encoded_df.groupby(greedy_solution, as_index=False)[
        target_variable_number
    ].mean()
    greedy_tuples = greedy_agg_df.to_numpy()

    brute_force_agg_df = encoded_df.groupby(bs_solution, as_index=False)[
        target_variable_number
    ].mean()
    brute_force_tuples = brute_force_agg_df.to_numpy()
...
results_sizes_df = build_deterministic_trie(
    trained_pipeline, training_features, path, "class", [3, 9, 15, 16], [3, 15, 16, 2]
)
...
data_folder = os.path.join(
    ...
    "experiments",
    "output",
    "brute_force_search_creditcard_sizes_df.csv",
)
