...
data_path = os.path.join(
    x_folder, "data", "paper_data", "nyc_rides", "nyc_rides_augmented_dirty.csv"
)
...
encoded_training_set = encoder.transform_dataset(
    x_featurized_inferdb.loc[:, subselection], [i for i in range(len(subselection))]
)

my_problem = Problem(
    encoded_training_set, y_train_pred, encoder.num_bins, "regression", 1
)
...
for index, row in tqdm(enumerate(X_test_arr)):
    ...
    preprocessed_instance = encoder.transform_single(
        instance, my_optimizer.greedy_solution
    )
...
results_list.append(
    ["InferDB", encoded_training_set.shape, model.__class__.__name__, error]
)
