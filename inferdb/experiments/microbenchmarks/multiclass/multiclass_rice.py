...
column_transformer = ColumnTransformer(
    transformers=[("num", numerical_transformer, list(X_train))],
    remainder="passthrough",
    n_jobs=-1,
)

pipeline = Pipeline(steps=[("column_transformer", column_transformer), ("clf", model)])
...
exp = Standalone(
    X_train, X_test, y_train, y_test, "rice", "multi-class", False, pipeline=pipeline
)

d = exp.create_report(cat_mask=[], with_pred=True)

export_path = os.path.join(
    exp_folder, "experiments", "output", exp.experiment_name + "_" + exp.model_name
)
d.to_csv(export_path + "_standalone.csv", index=False)


X_train_sub, X_test_sub = (
    X_train.loc[:, [training_features[i] for i in exp.solution]],
    X_test.loc[:, [training_features[i] for i in exp.solution]],
)
...
pipeline.named_steps.column_transformer.transformers[0] = (
    numerical_imputer[0],
    numerical_imputer[1],
    list(X_train_sub),
)
...
ConfusionMatrixDisplay.from_predictions(
    y_test, exp.y_pred_trie, display_labels=le.classes_, ax=axs[1]
)
ConfusionMatrixDisplay.from_predictions(
    y_test, y_pred, display_labels=le.classes_, ax=axs[0]
)
