...
class preprocessing_experiment:
    ...
    def insert_test_tuples(self, path_to_data, target_feature):
        ...
        test_input = np.hstack(
            (X_test.to_numpy(), y_test.reshape((y_test.shape[0], 1)))
        )
        ...
        cur.copy_expert(
            sql.SQL("COPY pgml.{table_name} FROM STDIN WITH CSV DELIMITER ';'").format(
    ...
    def insert_train_tuples(self, trained_pipeline, path_to_data, target_feature):
        ...
        try:
            train_input = np.hstack(
                (transformed_df, y_train.reshape((y_train.shape[0], 1)))
            )
        except ValueError:
        ...
        cur.copy_expert(
            "COPY pgml.nyc_rides_train FROM STDIN WITH CSV DELIMITER ';'", buffer
        )
        conn.commit()
    ...
    def create_featurizer(
        ...
        if pickup_encoder and dropoff_encoder and cluster_encoder and freq_mapper:
            ...
            for idc in range(number_of_pickup_clusters):
                if idc < number_of_pickup_clusters - 1:
                    pickup_cluster += (
                        ...
                        + str(pickup_encoder[0][idc][0])
                        + " and "
                        + str(pickup_encoder[0][idc][1])
                        + " AND pickup_longitude BETWEEN "
                        + str(pickup_encoder[1][idc][0])
                        + " and "
                        + str(pickup_encoder[1][idc][1])
                        + " THEN "
                        + str(idc)
                    )
                else:
            ...
            for idc in range(number_of_dropoff_clusters):
                if idc < number_of_dropoff_clusters - 1:
                    dropoff_cluster += (
                        " WHEN dropoff_latitude BETWEEN "
                        + str(dropoff_encoder[0][idc][0])
                        + " and "
                        + str(dropoff_encoder[0][idc][1])
                        + " AND dropoff_longitude BETWEEN "
                        + str(dropoff_encoder[1][idc][0])
                        + " and "
                        + str(dropoff_encoder[1][idc][1])
                        + " THEN "
                        + str(idc)
                    )
                else:
            ...
            subquery = sql.SQL(
                ...
                pickup_cluster=sql.SQL(pickup_cluster),
            ...
            for idc in range(number_clusters):
                if idc < number_clusters - 1:
                    clusters_cluster += (
                        " WHEN s.pickup_cluster IN "
                        + str(tuple(cluster_encoder[0][idc]))
                        + " AND s.dropoff_cluster IN "
                        + str(tuple(cluster_encoder[1][idc]))
                        + " THEN "
                        + str(idc)
                    )
                else:
            ...
            def create_route_freq_table():
                ...
                cur.execute(
                    "DROP TABLE IF EXISTS pgml.route_freq CASCADE; CREATE TABLE pgml.route_freq(cluster_combination TEXT, day TEXT, hour INTEGER, count REAL, PRIMARY KEY(cluster_combination, day, hour))"
                )
                conn.commit()
            ...
            def insert_route_freq_tuples(route_freq_mapper):
                ...
                tuples = [
                    (d[0], d[1], d[2], route_freq_mapper.get(d)["count"])
                    for d in route_freq_mapper
                ]
                ...
                cur.copy_expert(
                    sql.SQL("COPY pgml.route_freq FROM STDIN WITH CSV DELIMITER ';'"),
                    buffer,
                )
            ...
            featurizer_statement = sql.SQL(
                ...
                input_name=sql.SQL(input_name), limit_factor=sql.SQL(str(batch_size))
            )
        ...
        persist_statement = sql.SQL(
            """
                                    ...
                                    """
        ).format(name=sql.SQL(self.name), input_name=sql.SQL(input_name))
    ...
    def create_kv_featurizer(self, batch_size=None):
        ...
        featurizer_statement = sql.SQL(
            """
                                            ...
                                            $func$ LANGUAGE SQL STABLE;
                                        """
        ).format(input_name=sql.SQL(input_name), limit_factor=sql.SQL(str(batch_size)))
    ...
    def create_encode_scale_set(self, trained_pipeline):
        ...
        num_mask = trained_pipeline.named_steps["column_transformer"].transformers_[0][
            2
        ]
        cat_mask = trained_pipeline.named_steps["column_transformer"].transformers_[1][
            2
        ]
        ...
        scaler = (
            trained_pipeline.named_steps["column_transformer"]
            .transformers_[0][1]
            .named_steps["scaler"]
        )
        means = scaler.mean_
        ...
        for idf, feature in enumerate(categorical_feature_names):
            ...
            for idc, category in enumerate(unique_categories):
                if (
                    idf == len(categorical_feature_names) - 1
                    and idc == len(unique_categories) - 1
                ):
                    select_statement += (
                        " CASE WHEN "
                        + feature
        ...
        persist_statement = sql.SQL(
            """
                                    ...
                                    """
        ).format(function_name=sql.SQL(function_name), name=sql.SQL(self.name))
    ...
    def create_preprocessing_pipeline(self):
        ...
        function_statement = sql.SQL(
            """
                                    ...
                                    """
        ).format(input_name=sql.SQL(input_name), name=sql.SQL(self.name))
        ### Create function
        cur.execute(function_statement)
        conn.commit()
    ...
    def create_coef_table(self, trained_pipeline):
        ...
        insert_statement = (
            "INSERT INTO pgml." + input_name + "_coefficients (col_id, val) VALUES "
        )
    ...
    def create_scorer_function_lr(self):
        ...
        function_statement = sql.SQL(
            """--DROP FUNCTION IF EXISTS pgml.{input_name}_score;
    ...
    def create_nn_table(self, trained_pipeline):
        ...
        insert_statement = (
            """ INSERT INTO pgml.nn_matrix_"""
            + self.name
            + """ (id, row, col, val) VALUES """
        )
        ...
        for idc, i in enumerate(trained_pipeline[-1].intercepts_):
            ...
            for idx, bias in enumerate(i):
                tup = (idc, idx, 0, bias)
                if idc < len(trained_pipeline[-1].intercepts_) + len(
                    trained_pipeline[-1].coefs_
                ) - 1 and idx < len(i):
                    insert_statement += str(tup) + ","
                else:
                    insert_statement += str(tup) + ";"
    ...
    def create_acc_measure_function(self, scoring_function):
        ...
        query = sql.SQL(
            """ with scores as(select sqrt(sum((ln(GREATEST(p.prediction, 0) + 1) - ln(t.target + 1))^2)/count(p.*)) as rmsle
    ...
    def get_kv_tuples(
        self, trained_pipeline, purpose, path_to_data, target_feature, persist
    ):
        ...
        if not persist:
            ...
            self.persist_encoder_kv_tuples(
                tuples, encoder, features_names, my_optimizer.greedy_solution
            )
    ...
    def persist_encoder_kv_tuples(self, tuples, encoder, features_names, solution):
        ...
        if self.featurizer.depth == "deep":
            ...
            encoder_path = os.path.join(
                ...
                + "_"
                + (self.model.__class__.__name__).lower()
                + "_complex_encoder.joblib",
            )
            features_names_path = os.path.join(
                path,
                self.name
                + "_"
                + (self.model.__class__.__name__).lower()
                + "_complex_features_names.joblib",
            )
    ...
    def create_kv_table(self):
        ...
        cur.execute(
            sql.SQL(
                "DROP TABLE IF EXISTS pgml.{table_name} CASCADE; CREATE TABLE pgml.{table_name} (key TEXT NOT NULL, value NUMERIC NOT NULL)"
    ...
    def create_index(self):
        ...
        index_name = (
            self.name
            + "_"
            + (self.pipeline[-1].__class__.__name__).lower()
            + "_"
            + "index"
        )
        ...
        cur.execute(
            sql.SQL(
                "DROP INDEX IF EXISTS {index_name} CASCADE;CREATE INDEX {index_name} ON pgml.{table_name} using spgist(key); analyze;"
            ).format(
    ...
    def create_preproccesing_function(self, encoder, features_names, solution_indices):
        ...
        function_statement = sql.SQL(
            """ --DROP FUNCTION IF EXISTS pgml.{input_name}_translate CASCADE;
    ...
    def create_scorer_function_pgml(self):
        ...
        function_statement = sql.SQL(
            """
                                    ...
                                    """
        ).format(input_name=sql.SQL(input_name), experiment_name=sql.SQL(self.name))
    ...
    def create_measure_function(self, function):
        ...
        function_statement = sql.SQL(
            """
                    ...
                    $$ LANGUAGE plpgsql;
                """
        ).format(input_name=sql.SQL(input_name), function=sql.SQL(function))
    ...
    def generate_report_function_pg(self):
        ...
        select_statement_model = sql.SQL(
            """ , pgml.measure_acc_{input_name}_score() as rmsle"""
        ).format(input_name=sql.SQL(input_name))
        select_statement_index = sql.SQL(
            """, pgml.measure_acc_{input_name}_score_kv() as rmsle"""
        ).format(input_name=sql.SQL(input_name))
        select_statement_pgml = sql.SQL(
            """, pgml.measure_acc_{input_name}_score_pgml() as rmsle"""
        ).format(input_name=sql.SQL(input_name))
    ...
    def create_report_pg(self, batch_size):
        ...
        size_effectiveness_query = sql.SQL(
            """ select * from pgml.{input_name}_report()"""
        ).format(input_name=sql.SQL(input_name))
        ...
        impute_time_query = sql.SQL(
            """ analyze; select * from pgml.measure_featurizer_runtime_{input_name}()"""
        ).format(input_name=sql.SQL(input_name))
        encode_scale_time_query = sql.SQL(
            """ analyze; select * from pgml.measure_encode_scale_set_runtime_{input_name}()"""
        ).format(input_name=sql.SQL(input_name))
        scoring_time_query = sql.SQL(
            """ analyze; select * from pgml.measure_score_runtime_{input_name}()"""
        ).format(input_name=sql.SQL(input_name))
        ...
        impute_time_query = sql.SQL(
            """ analyze; select * from pgml.measure_featurizer_kv_runtime_{input_name}()"""
        ).format(input_name=sql.SQL(input_name))
        translate_time_query = sql.SQL(
            """ analyze; select * from pgml.measure_translate_runtime_{input_name}()"""
        ).format(input_name=sql.SQL(input_name))
        scoring_time_query = sql.SQL(
            """ analyze; select * from pgml.measure_score_kv_runtime_{input_name}()"""
        ).format(input_name=sql.SQL(input_name))
        ...
        scoring_time_query = sql.SQL(
            """ analyze; select * from pgml.measure_score_pgml_runtime_{input_name}()"""
        ).format(input_name=sql.SQL(input_name))
    ...
    def create_standalone_index(self, trained_pipeline, path_to_data, target_feature):
        ...
        st = time.time()
        tuples, encoder, features_names, solution = self.get_kv_tuples(
            trained_pipeline, "standalone", path_to_data, target_feature, False
        )
    ...
    def perform_index_inference(self, trained_pipeline, path_to_data, target_feature):
        ...
        model_index, time_to_populate_index, encoder, solution = (
            self.create_standalone_index(trained_pipeline, path_to_data, target_feature)
        )
        ...
        index_error = mean_squared_log_error(
            y_test[indices], y_pred_trie, squared=False
        )
    ...
    def create_report(self, trained_pipeline, path_to_data, target_feature):
        ...
        model_avg_prep_runtimes, model_avg_scoring_runtimes, model_size, model_error = (
            self.perform_model_inference(trained_pipeline, path_to_data, target_feature)
        )
        (
            index_avg_prep_runtimes,
            index_avg_scoring_runtimes,
            ...
            index_error,
            index_time,
        ) = self.perform_index_inference(trained_pipeline, path_to_data, target_feature)
        summary = [
            [
                self.name,
                (self.pipeline[-1].__class__.__name__).lower(),
