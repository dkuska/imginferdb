...
for m in models:
    ...
    for p in pipelines:
        ...
        #### train
        test.create_train_table(
            pipeline, str(project_folder) + "/data/nyc_rides/train.csv", "trip_duration"
        )
        test.insert_train_tuples(
            pipeline, str(project_folder) + "/data/nyc_rides/train.csv", "trip_duration"
        )
        ...
        test.insert_test_tuples(
            str(project_folder) + "/data/nyc_rides/train.csv", "trip_duration"
        )
        test.geo_distance()
        ...
        for b in batch_sizes:
            ...
            if p == "complex":
                dropoff_encoder = pipeline.named_steps[
                    "featurizer"
                ].dropoff_encoder.splits
                pickup_encoder = pipeline.named_steps[
                    "featurizer"
                ].pickup_encoder.splits
                route_freq_dict = pipeline.named_steps["featurizer"].route_freq_mappers
                clusters_encoder = pipeline.named_steps[
                    "featurizer"
                ].clusters_encoder.splits
                test.create_featurizer(
                    pickup_encoder,
                    dropoff_encoder,
                    clusters_encoder,
                    route_freq_dict,
