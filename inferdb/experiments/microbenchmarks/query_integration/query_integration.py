...
def create_pg_artifacts(model, pipeline_type, batch_size):
    ...
    test.create_featurizer(
        pickup_encoder, dropoff_encoder, clusters_encoder, route_freq_dict, batch_size
    )
...
def set_inferdb_query(p_limit, model):
    ...
    ).format(model=sql.SQL(model), p_limit=sql.SQL(str(p_limit)))
    return query
