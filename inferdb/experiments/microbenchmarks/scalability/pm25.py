...
from transpiler import MADLIB, PGML, InferDB, SQLmodel

data_path = os.path.join(x_folder, "data", "pm", "pm_2016_2019_augmented.csv")
write_path = os.path.join(
    x_folder, "experiments", "output", "scalability_pm25_pgml.csv"
)
...
lgbm = LGBMRegressor(
    n_estimators=350, n_jobs=-1, objective="regression", reg_lambda=1, reg_alpha=1
)
...
for threshold in train_test_thresholds:
    ...
    X_test = df.loc[
        (df.index > threshold[0]) & (df.index <= threshold[1]), training_features
    ].bfill()
    X_train = X_train.fillna(0)
    X_test = X_test.fillna(0)
    y_train = df.loc[df.index < threshold[0], "DS_PM_pred"].to_numpy()
    y_test = df.loc[
        (df.index > threshold[0]) & (df.index <= threshold[1]), "DS_PM_pred"
    ].to_numpy()

    X_train, X_test = X_train.to_numpy(), X_test.to_numpy()

    sample_indices = np.random.randint(
        X_train.shape[0], size=round(X_train.shape[0] * 0.3)
    )
    ...
    for model in PGML_models:
        ...
        model_parameters = PGML_models_parameters[model.__class__.__name__][
            "parameters"
        ]
        ...
        summary_df = exp.create_report_pg(
            model_name_pgml, PGML_models_parameters[model_name]["parameters"]
        )
