...
def create_test_pg(
    path_to_data, target_feature, trained_pipeline, batch_sizes, index_sizes
):
    ...
    for l in index_sizes:
        for b in batch_sizes:
            ...
            for iteration in range(5):
                ...
                cur.execute(
                    sql.SQL(
                        """explain analyze select case when k2.value is not null then k2.value else pgml.prefix_search(k1.key, 'kv_table_test') end as prediction
                                            from (select * from pgml.test_table limit {batch}) k1
                                            left join pgml.kv_table_test k2
                                            on k1.key = k2.key """
                    ).format(batch=sql.SQL(str(b)))
    ...
    results_df = pd.DataFrame(
        ...
        columns=[
            ...
            "Iteration",
            "Prediction Latency [ms] qpo",
            "Query Plan Or.",
            "Prediction Latency [ms] mod",
            "Query Plan mod.",
        ],
    )
...
pg_df = create_test_pg(
    path, "trip_duration", trained_pipeline, [1, 10, 100, 1000, 10000, 100000], [6, 10]
)
data_folder = os.path.join(
    project_folder, "experiments", "output", "test_data_size_df.csv"
)
