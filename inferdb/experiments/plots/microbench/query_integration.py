...
def plot_curves(wspace, fontsize):
    ...
    for idm, m in enumerate(df["Algorithm"].unique()):
        ...
        agg_df = (
            df.loc[df["Algorithm"] == m, :]
            .groupby(["Competitor", "Limit"], as_index=False)
            .agg({"Runtime": ["mean", "std"]})
        )

        #### INFERDB Numbers
        inferdb_y_array = agg_df.loc[
            agg_df[("Competitor", "")] == "InferDB", ("Runtime", "mean")
        ].to_numpy()
        print(inferdb_y_array)
        inferdb_error = agg_df.loc[
            agg_df[("Competitor", "")] == "InferDB", ("Runtime", "std")
        ].to_numpy()

        #### SQL Model Numbers
        sql_y_array = agg_df.loc[
            agg_df[("Competitor", "")] == "SQL Model", ("Runtime", "mean")
        ].to_numpy()
        sql_error = agg_df.loc[
            agg_df[("Competitor", "")] == "SQL Model", ("Runtime", "std")
        ].to_numpy()

        if m == "linearregression":

            #### SQL Model Numbers
            pgml_y_array = agg_df.loc[
                agg_df[("Competitor", "")] == "PGML", ("Runtime", "mean")
            ].to_numpy()
            pgml_error = agg_df.loc[
                agg_df[("Competitor", "")] == "PGML", ("Runtime", "std")
            ].to_numpy()
            ...
            ax[idm].plot(
                ...
                label="SQL Model",
                linewidth=1,
                color=param_dict["colors"]["SQL Model"],
            )
            ax[idm].plot(
                x_array,
                inferdb_y_array,
                label="InferDB",
                linewidth=1,
                color=param_dict["colors"]["InferDB"],
            )
            ...
            ax[idm].errorbar(
                ...
                yerr=sql_error,
                fmt="none",
                ecolor=param_dict["colors"]["SQL Model"],
                elinewidth=1,
            )
            ax[idm].errorbar(
                x_array,
                ...
                yerr=inferdb_error,
                fmt="none",
                ecolor=param_dict["colors"]["InferDB"],
                elinewidth=1,
            )
            ax[idm].errorbar(
                x_array,
