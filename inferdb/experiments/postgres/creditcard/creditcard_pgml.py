...
def get_experiment_data():
    ...
    data_path = os.path.join(
        x_folder, "data", "paper_data", "creditcard", "creditcard.csv"
    )
...
def get_pipeline(model):
    ...
    column_transformer = ColumnTransformer(
        transformers=[("num", numerical_transformer, [i for i in range(30)])],
    ...
    if model.__class__.__name__ in tree_based_models:
        ...
        numerical_scaler = pipeline.named_steps.column_transformer.transformers[0]
        pipeline.named_steps.column_transformer.transformers = [
            ("num", numerical_scaler[1], [i for i in range(30)])
        ]
...
def creditcard_experiment(iterations=5, paper_models=False):
    ...
    nn = MLPClassifier(
        hidden_layer_sizes=(30,), max_iter=10000, activation="logistic", alpha=0.005
    )
    lgbm = LGBMClassifier(
        n_estimators=350,
        n_jobs=-1,
