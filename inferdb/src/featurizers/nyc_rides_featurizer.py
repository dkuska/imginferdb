...
class NYC_Featurizer(BaseEstimator, TransformerMixin):
    ...
    def impute(self, feature, dtype):
        ...
        if dtype == "datetime":
            ...
            mean = (
                np.array(feature_array, dtype="datetime64[s]")
                .view("i8")
                .mean()
                .astype("datetime64[s]")
            )
    ...
    def cal_pickup_bearing_degrees(
        self, pickup_lat, pickup_long, dropoff_lat, dropoff_long
    ):
    ...
    def cal_dropoff_bearing_degrees(
        self, pickup_lat, pickup_long, dropoff_lat, dropoff_long
    ):
    ...
    def fit(self, X, y):
        ...
        ######## DISTANCE
        X_["geo_distance"] = X_.apply(
            lambda x: self.cal_distance(
                x["pickup_latitude"],
                x["pickup_longitude"],
                x["dropoff_latitude"],
                x["dropoff_longitude"],
        ...
        if self.depth == "deep":
            ...
            for feature in [
                ...
                X_train = X_.iloc[
                    ...
                    [
                        idx
                        for idx, i in enumerate(list(X_))
                        if i != feature
                        and (i in self.num_features or i in self.cat_features)
                    ],
                ]
                y_train = X_.iloc[
                    outlier_indices[0], list(X_).index(feature)
                ].to_numpy()

                training_features = [
                    i for i in list(X_) if i != feature and i in self.num_features
                ]
            ...
            X_["pickup_cluster"] = self.kmeans.predict(
                X_[["pickup_latitude", "pickup_longitude"]].values
            )
            X_["dropoff_cluster"] = self.kmeans.predict(
                X_[["dropoff_latitude", "dropoff_longitude"]].values
            )
            ...
            speed_df = X_.groupby(["pickup_cluster", "dropoff_cluster"])["speed"].agg(
                ["mean"]
            )
            ...
            agg_clusters_avg_trip_duration_df = clusters_avg_trip_duration_df.groupby(
                ["pickup_cluster", "dropoff_cluster"]
            )["trip_duration"].agg(["mean"])
            self.route_avg_trip_duration = agg_clusters_avg_trip_duration_df.to_dict(
                "index"
            )

            traffic_df = X_.groupby(["pickup_cluster", "dropoff_cluster"])["id"].agg(
                ["count"]
            )
            ...
            steps_df = X_.groupby(["pickup_cluster", "dropoff_cluster"])[
                "number_of_steps"
            ].agg(["mean"])
            self.route_avg_steps = steps_df.to_dict("index")

            travel_time_df = X_.groupby(["pickup_cluster", "dropoff_cluster"])[
                "total_travel_time"
            ].agg(["mean"])
            self.route_avg_travel_time = travel_time_df.to_dict("index")

            distance_df = X_.groupby(["pickup_cluster", "dropoff_cluster"])[
                "total_distance"
            ].agg(["mean"])
            self.route_avg_distance = distance_df.to_dict("index")

            mappers_df = X_.groupby(["pickup_cluster", "dropoff_cluster"])[
                [
                    "total_distance",
                    "total_travel_time",
                    "number_of_steps",
    ...
    def transform(self, X):
        ...
        X_["pickup_dt"] = (
            X_["pickup_datetime"] - X_["pickup_datetime"].min()
        ).dt.total_seconds()
        ...
        X_["is_weekend"] = X_.apply(
            lambda x: 1 if x["pickup_weekday"] in (5, 6) else 0, axis=1
        )

        if self.depth == "deep":
            X_["arc"] = X_.apply(
                lambda x: self.cal_arc(
                    x["pickup_latitude"],
                    x["pickup_longitude"],
                    x["dropoff_latitude"],
                    x["dropoff_longitude"],
            ...
            X_["pickup_bearing"] = X_.apply(
                lambda x: self.cal_pickup_bearing_degrees(
                    x["pickup_latitude"],
                    x["pickup_longitude"],
                    x["dropoff_latitude"],
                    x["dropoff_longitude"],
            ...
            X_["dropoff_bearing"] = X_.apply(
                lambda x: self.cal_dropoff_bearing_degrees(
                    x["pickup_latitude"],
                    x["pickup_longitude"],
                    x["dropoff_latitude"],
                    x["dropoff_longitude"],
            ...
            def get_route_freq(X_):
                ...
                try:
                    value = self.route_freq_mappers[
                        (X_["pickup_cluster"], X_["dropoff_cluster"])
                    ]["count"]
            ...
            def get_route_avg_duration(X_):
                try:
                    value = self.route_avg_trip_duration[
                        (X_["pickup_cluster"], X_["dropoff_cluster"])
                    ]["mean"]
                    if np.isfinite(value):
                        return value
            ...
            def get_route_avg_travel_time(X_):
                try:
                    value = self.route_avg_travel_time[
                        (X_["pickup_cluster"], X_["dropoff_cluster"])
                    ]["mean"]
                    if np.isfinite(value):
                        return value
            ...
            def predict_outlier_passenger(X_):
                ...
                X_test = instance[
                    self.outlier_training_features["passenger_count"]
                ].to_numpy()[0]
                pred = self.outlier_imputers["passenger_count"][0] + np.dot(
                    self.outlier_imputers["passenger_count"][1], X_test
                )
            ...
            def predict_outlier_distance(X_):
                ...
                instance = X_.to_frame().transpose()
                X_test = instance[
                    self.outlier_training_features["distance"]
                ].to_numpy()[0]
                pred = self.outlier_imputers["distance"][0] + np.dot(
                    self.outlier_imputers["distance"][1], X_test
                )
            ...
            def predict_outlier_geo_distance(X_):
                ...
                instance = X_.to_frame().transpose()
                X_test = instance[
                    self.outlier_training_features["geo_distance"]
                ].to_numpy()[0]
                pred = self.outlier_imputers["geo_distance"][0] + np.dot(
                    self.outlier_imputers["geo_distance"][1], X_test
                )
            ...
            X_["passenger_count"] = X_.apply(
                lambda x: (
                    predict_outlier_passenger(x)
                    if x["passenger_count"]
                    > self.outlier_boundaries["passenger_count"][1]
                    or x["passenger_count"]
                    < self.outlier_boundaries["passenger_count"][0]
                    else x["passenger_count"]
                ),
                axis=1,
            )
            X_["distance"] = X_.apply(
                lambda x: (
                    predict_outlier_distance(x)
                    if x["distance"] > self.outlier_boundaries["distance"][1]
                    or x["distance"] < self.outlier_boundaries["distance"][0]
                    else x["distance"]
                ),
                axis=1,
            )
            X_["geo_distance"] = X_.apply(
                lambda x: (
                    predict_outlier_geo_distance(x)
                    if x["geo_distance"] > self.outlier_boundaries["geo_distance"][1]
                    or x["geo_distance"] < self.outlier_boundaries["geo_distance"][0]
                    else x["geo_distance"]
                ),
                axis=1,
            )
            ...
            X_["avg_trip_duration"] = X_.apply(
                lambda x: get_route_avg_duration(x), axis=1
            )
            ...
            X_["avg_travel_time"] = X_.apply(
                lambda x: get_route_avg_travel_time(x), axis=1
            )
            X_["avg_distance"] = X_.apply(lambda x: get_route_avg_distance(x), axis=1)

            X_["pickup_pca0"] = self.pca.transform(
                X_[["pickup_latitude", "pickup_longitude"]].values
            )[:, 0]
            X_["pickup_pca1"] = self.pca.transform(
                X_[["pickup_latitude", "pickup_longitude"]].values
            )[:, 1]
            X_["dropoff_pca0"] = self.pca.transform(
                X_[["dropoff_latitude", "dropoff_longitude"]].values
            )[:, 0]
            X_["dropoff_pca1"] = self.pca.transform(
                X_[["dropoff_latitude", "dropoff_longitude"]].values
            )[:, 1]
            X_["pca_manhattan"] = np.abs(
                X_["dropoff_pca1"] - X_["pickup_pca1"]
            ) + np.abs(X_["dropoff_pca0"] - X_["pickup_pca0"])

        X_.rename(
            columns={
                "primary": "primarie",
                "nTrafficSignals": "ntrafficsignals",
                "nCrossing": "ncrossing",
                "nStop": "nstop",
        ...
        self.features_to_keep = [
            ...
            if i in self.cat_features or i in self.num_features
        ]
