    ...
    def __init__(self, encoded_x, y, encoder_num_bins, task, sample_factor) -> None:
        ...
        if self.sample_factor < 1:
            sample_indices = np.random.choice(
                self.encoded_x.shape[0],
                round(self.encoded_x.shape[0] * self.sample_factor),
            )
        ...
        if self.task in ("regression", "classification"):
            ...
            global_events = self.df.groupby(self.target_variable_number)[
                self.target_variable_number
            ].agg(["count"])
    ...
    def set_information_value(self):
        ...
        for idx in range(self.encoded_x.shape[1]):
            if self.task == "classification":
                agg_df = self.df.groupby(idx)[self.target_variable_number].agg(
                    ["count", "sum"]
                )
                agg_df.loc[:, "woe"] = agg_df.apply(
                    lambda x: (
                        (
                            log((x["count"] - x["sum"]) + self.adjustement_factor)
                            - log(self.global_nonevents)
                        )
                        - (
                            log(x["sum"] + self.adjustement_factor)
                            - log(self.global_events)
                        )
                    )
                    * (
                        (x["count"] - x["sum"]) / self.global_nonevents
                        - x["sum"] / self.global_events
                    ),
                    axis=1,
                )
                ...
                agg_df = (
                    local_instances.set_index(idx)
                    .join(class_instances.set_index(idx), lsuffix="_other")
                    .fillna(0)
                )
                agg_df.loc[:, "woe"] = agg_df.apply(
                    lambda x: (
                        (
                            log((x["size_other"] - x["size"]) + self.adjustement_factor)
                            - log(
                                sum(class_agg)
                                - class_agg[x[self.target_variable_number]]
                                + self.adjustement_factor
                            )
                        )
                        - (
                            log(x["size"] + self.adjustement_factor)
                            - log(
                                class_agg[x[self.target_variable_number]]
                                + self.adjustement_factor
                            )
                    ...
                    * (
                        self.weird_division(
                            (x["size_other"] - x["size"]),
                            sum(class_agg) - class_agg[x[self.target_variable_number]],
                        )
                        - self.weird_division(
                            x["size"], class_agg[x[self.target_variable_number]]
                        )
                    ),
                    axis=1,
                )
                ...
                agg_df.loc[:, "woe"] = agg_df.apply(
                    lambda x: abs(x["mean"] - self.global_mean)
                    * (x["count"] / self.global_instances),
                    axis=1,
                )
                iv_list.append(agg_df["woe"].sum())
...
class Optimizer:
    ...
    def __init__(self, problem, sample_factor) -> None:
        ...
        self.sample_factor = sample_factor
        self.class_agg = self.problem.df.groupby(
            self.problem.target_variable_number
        ).size()
    ...
    def compute_iv(self, index):
        ...
        if self.problem.task == "classification":
            agg_df = self.problem.df.groupby(index)[
                self.problem.target_variable_number
            ].agg(["count", "sum"])
            agg_df.loc[:, "woe"] = agg_df.apply(
                lambda x: (
                    (
                        log((x["count"] - x["sum"]) + self.problem.adjustement_factor)
                        - log(self.problem.global_nonevents)
                    )
                    - (
                        log(x["sum"] + self.problem.adjustement_factor)
                        - log(self.problem.global_events)
                    )
                )
                * (
                    (x["count"] - x["sum"]) / self.problem.global_nonevents
                    - x["sum"] / self.problem.global_events
                ),
                axis=1,
            )
            iv = agg_df["woe"].sum()
            ...
            agg_df.loc[:, "woe"] = agg_df.apply(
                lambda x: abs(x["mean"] - self.problem.global_mean)
                * (x["count"] / self.problem.global_instances),
                axis=1,
            )
            iv = agg_df["woe"].sum()
            ...
            class_instances = self.problem.df.groupby(
                index_classes, as_index=False
            ).size()

            agg_df = (
                local_instances.set_index(index)
                .join(class_instances.set_index(index), lsuffix="_other")
                .fillna(0)
            )
            agg_df.loc[:, "woe"] = agg_df.apply(
                lambda x: (
                    (
                        log(
                            (x["size_other"] - x["size"])
                            + self.problem.adjustement_factor
                        )
                        - log(
                            sum(self.class_agg)
                            - self.class_agg[x[self.problem.target_variable_number]]
                            + self.problem.adjustement_factor
                        )
                    )
                    - (
                        log(x["size"] + self.problem.adjustement_factor)
                        - log(
                            self.class_agg[x[self.problem.target_variable_number]]
                            + self.problem.adjustement_factor
                        )
    ...
    def greedy_search(self):
        ...
        set_of_candidates = np.argsort(self.problem.costs_array)[
            ::-1
        ]  ## --> sort features by IV desc

        index = []
        information_value = 0
        if self.sample_factor < 1:
            sample_indices = np.random.choice(
                self.problem.df.shape[0],
                round(self.problem.df.shape[0] * self.sample_factor),
            )
        ...
        for idf, feature_index in tqdm(enumerate(set_of_candidates)):
            ...
            if iv <= information_value * 1.002:
                ...
                index_bins = np.argsort(
                    self.problem.bin_array[index]
                )  ## -> If we sort by number of bins asc, the index storage decreases.
    ...
    def sensitivity(self, sample_factor=1, index_size=None):
        ...
        if sample_factor < 1:
            sample_indices = np.random.choice(
                df.shape[0], round(df.shape[0] * sample_factor)
            )
